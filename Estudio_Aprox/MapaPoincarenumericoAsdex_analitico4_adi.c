...
int ii=0,jj=0;double B1r1[N_r*N_z],B1z1[N_r*N_z],B1r2[N_r*N_z],B1z2[N_r*N_z];
...
/* 	for(i=0;i<(N_r);i++){ */
	...
	for(i=0;i<N_z;i++){
	  for(j=0;j<N_r;j++){
	    fscanf(in,"%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n",&aux,&aux,&B1r1[i*N_r+j],&B1r2[i*N_r+j],&B1z1[i*N_r+j],&B1z2[i*N_r+j]);
		//printf("%e \n",Br1[i][j]);
