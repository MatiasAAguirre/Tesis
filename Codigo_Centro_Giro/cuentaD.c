...
void cuentaD(double vpar) {
  ...
  ta = 1/(Omega*180/3.14159);      // Período de Ciclotrón en segundos.
  ...
  Rl = dsqrt(1-vpar0*vpar0)*a; //Radio de Larmor en metros.

  printf("------Datos Varios------\n");
  //printf("frec %e \n",2*3.14159*frec/Omega);
  //printf("gamma %f omega %e gamma/omega %e \n",gam, Omega, Gamma/Omega);
  printf("Gamma = %f\n", gam);
  printf("Omega = %e rad/s, o ta = %e s\n", Omega, ta);
  //printf("v0(m/s) %f E(keV) %f  %f \n",v0,E/1.602e-19/1000., v0*100.e-6);
  printf("Radio de Larmor = %f cm\n", Rl*100);
